...
本仓库在教程版 GeeRPC 的基础上，又继续补了一些更适合项目展示和面试讲解的能力：

- 负载均衡扩展
  - 除随机、轮询外，还支持加权轮询、最少连接数和一致性哈希按 key 路由。
- 可观测性
  - 服务端支持 HTML debug 页面和 JSON stats 导出。
  - 客户端和连接池支持统计快照。
- 客户端基础容灾
  - `XClient.Call` 支持显式开启的重试与故障转移。
  - 默认保持关闭，不改变原有行为。

其中，重试与故障转移当前只处理连接/节点级失败，不处理业务错误，也不会自动接入 `CallByKey` 和 `Broadcast`。
